...
class ControleAccesTest(unittest.TestCase):
    ...
    def test_badge_admin_dans_horaire(self):
        # ÉTANT DONNÉ une Porte reliée à un Lecteur admin, ayant détecté un Badge valide pendant les heures d'ouverture
        self.lecteur.simuler_detection_badge()
        self.lecteur.rendre_admin()

        self.moteur.associer(self.lecteur, self.porte)

        # QUAND le Moteur d'Ouverture effectue une interrogation des lecteurs pendant les heures d'ouverture
        heure_dans_plage = time(10)
        self.moteur.interroger(heure_actuelle=heure_dans_plage)

        # ALORS le signal d'ouverture est envoyé à la porte
        self.assertTrue(self.porte.ouverture_demandee)      

    def test_badge_normal_dans_horaire(self):
        # ÉTANT DONNÉ une Porte reliée à un Lecteur non admin, ayant détecté un Badge valide pendant les heures d'ouverture
        self.lecteur.simuler_detection_badge()
    ...
    def test_admin_peut_ouvrir_toutes_les_portes(self):
        # ÉTANT DONNÉ un Lecteur admin pouvant ouvrir plusieurs portes
        porte1 = PorteTest()
        porte2 = PorteTest()
        ...
        # ALORS le signal d'ouverture est envoyé à toutes les portes
        self.assertTrue(porte1.ouverture_demandee)
        self.assertTrue(porte2.ouverture_demandee)  

    def test_non_admin_peut_ouvrir_une_porte_pendant_horaire(self):
        # ÉTANT DONNÉ un Lecteur non admin pouvant ouvrir une porte pendant les heures d'ouverture
        porte1 = PorteTest()
        self.lecteur.simuler_detection_badge()
    ...
    def test_admin_et_non_admin_lecteurs(self):
        # ÉTANT DONNÉ une Porte reliée à un Lecteur admin et un Lecteur non admin
        porte = PorteTest()
        lecteur_admin = LecteurTest()
        lecteur_admin.simuler_detection_badge()
        lecteur_admin.rendre_admin()

        lecteur_non_admin = LecteurTest()
        lecteur_non_admin.simuler_detection_badge()

        self.moteur.associer(lecteur_admin, porte)
        self.moteur.associer(lecteur_non_admin, porte)
